...
router.get('/', cacheMiddleware(120), (req, res) => {
  ...
  const result = parties.map(p => {
    ...
    return { ...p, states_json: JSON.parse(p.states_json||'[]'), prev:{promises:prev,stats:cnt(prev)}, curr:{promises:curr,stats:cnt(curr)} };
  });
...
router.get('/:slug', cacheMiddleware(120), (req, res) => {
  ...
  `).all(party.name);
  res.json({ ...party, states_json: JSON.parse(party.states_json||'[]'), prev:{promises:prev,stats:cnt(prev)}, curr:{promises:curr,stats:cnt(curr)}, politicians:pols });
});

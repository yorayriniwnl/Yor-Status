...
router.post('/', authenticate, requireMod, (req, res) => {
  ...
  const cleanOptions = Array.isArray(options)
    ...
    : [];
  if (!title || cleanOptions.length < 2) return res.status(400).json({ error: 'title and at least 2 options required' });
  if (politician_id && !db.prepare(`SELECT id FROM politicians WHERE id=?`).get(politician_id)) {
  ...
  for (const opt of cleanOptions) db.prepare(`INSERT INTO poll_options (poll_id,text) VALUES (?,?)`).run(result.lastInsertRowid, opt);
  clearPollCaches();
...
router.post('/:id/vote', optionalAuth, (req, res) => {
  ...
  const poll = db.prepare(`SELECT * FROM polls WHERE id=? AND is_active=1 AND (ends_at IS NULL OR datetime(ends_at) > datetime('now'))`).get(req.params.id);
  ...
  try {
    db.transaction(() => {
      db.prepare(`INSERT INTO poll_votes (poll_id,option_id,session_id,user_id) VALUES (?,?,?,?)`).run(poll.id, option_id, sessId, req.user?.id || null);
    ...
    global.io?.emit('poll:update', { id: poll.id, options: updated });
    clearPollCaches();
    res.json({ ok: true });
  } catch(e) {
